...
namespace Coimbra.BuildManagement.Global
{
    internal static class GlobalSettingsProvider
    {
        ...
        internal static class General
        {
            ...
            private static readonly GUIContent UseProjectFolderAsBuildNameLabel = new GUIContent("Use Project Folder As Build Name*", "If not checked, the build name metadata will be the same as the product name.");
            private static readonly GUIContent ShowSplashScreenLabel = new GUIContent("Show Splash Screen*", "This setting will only take effect if you are a Plus/Pro subscriber. Useful if working on a project in which not all developers have an active subscription.");
            private static readonly GUIContent ShowUnityLogoLabel = new GUIContent("Show Unity Logo*", "This setting will only take effect if you are a Plus/Pro subscriber. Useful when working on a project in which not all developers have an active subscription.");
            private static readonly GUIContent UseUtcNowAsIosBuildNumberLabel = new GUIContent("Use Utc Now as iOS Build Number*", "If checked, the iOS Build Number will be the build time in UTC (Coordinated Universal Time). The build number will be in the format 'yyyy.MMdd.HHmm' thus ensuring an ever-increasing and ever-changing Build Number.");
            private static readonly GUIContent UseUtcNowAsMacBuildNumberLabel = new GUIContent("Use Utc Now as Mac Build Number*", "If checked, the Mac Build Number will be the build time in UTC (Coordinated Universal Time). The build number will be in the format 'yyyy.MMdd.HHmm' thus ensuring an ever-increasing and ever-changing Build Number.");
        ...
        internal static class CloudBuild
        {
            ...
            private static readonly GUIContent AppendBuildNumberToVersionLabel = new GUIContent("Append Build Number to Version*", "If checked, it will append the build number to the end of the version. This will ensure an ever-increasing and ever-changing Version.");
            private static readonly GUIContent BuildNumberExpressionLabel = new GUIContent("Build Number Expression*", $"Use {BuildNumberPlaceholder} symbol as the build number placeholder. If the expression is not valid, the result will be the Build Number.");
            private static readonly GUIContent UseBuildStartTimeAsUtcNowLabel = new GUIContent("Use Build Start Time as Utc Now*", "If checked, it will override the UTC (Coordinated Universal Time) information with the actual Build Start Time information from the Unity Cloud Build.");
            private static readonly GUIContent SumBuildNumberToBundleVersionCodeLabel = new GUIContent("Sum Build Number to Bundle Version Code*", "If checked, the Android Bundle Version Code will be a sum of the current Bundle Version Code with the Unity Cloud Build build number. This will ensure an ever-increasing and ever-changing Bundle Version Code.");
        ...
        {
            private static readonly GUIContent AllowFallbackToMonoLabel = new GUIContent("Allow Fallback To Mono*", "If IL2PP is the preferred scripting backend and it is not supported for the current selected platform in your machine it will automatically fallback to Mono.");
        ...
        [NotNull] internal static Settings Settings => _settings ?? (_settings = new Settings(BuildManagerUtility.PackageName));
